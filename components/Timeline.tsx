        ...
        try {
          ...
          return (
            <div
              ...
              className={`absolute h-8 top-1/2 -translate-y-1/2 transition-all rounded-sm
                ${range.type === 'preferred' 
                  ? 'bg-cred-accent/80 border-y border-cred-accent/40 hover:bg-cred-accent hover:shadow-[0_0_15px_rgba(225,255,89,0.4)] hover:z-10' 
                  : 'bg-zinc-800/80 border border-zinc-700 bg-[url("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAYAAACp8Z5+AAAAIklEQVQIW2NkQAKrVq36zwjjgzhhYWGMYAEYB8RmROaABADeOQ8CXl/xfgAAAABJRU5ErkJggg==")] opacity-50'}
              `}
    ...
    return daysArray.map((dayDate, i) => {
      ...
      return (
        <div 
          ...
          className="absolute top-0 bottom-0 flex flex-col justify-center border-l-2 border-dashed border-zinc-700/50 pl-2 pointer-events-none select-none group-hover:border-zinc-600"
          style={{ left: px }}
        >
          <div className="bg-zinc-900/80 text-[10px] font-bold text-zinc-400 px-2 py-1 rounded border border-zinc-800 whitespace-nowrap backdrop-blur-sm">
             <span className="text-zinc-500 mr-1">{displayDay}</span>
    ...
    <div className="flex flex-col w-full h-full bg-cred-bg overflow-hidden relative select-none">
      ...
      <div className="absolute left-0 top-[60px] bottom-0 w-56 bg-cred-bg border-r border-cred-border z-30 flex flex-col shadow-2xl">
        <div className="h-20 border-b border-cred-border flex items-center px-4 bg-cred-card/50 backdrop-blur shrink-0">
          <div className="w-8 h-8 bg-zinc-800 flex items-center justify-center text-cred-accent mr-3 border border-zinc-700 rounded-sm">
            <Users size={16} />
          </div>
          ...
          {members.map(member => {
            ...
            return (
              <div 
                ...
                className="h-20 border-b border-cred-border/50 flex items-center px-4 hover:bg-zinc-900/50 cursor-pointer group transition-colors relative"
              >
                <div className="w-8 h-8 bg-zinc-800 flex items-center justify-center text-xs font-mono font-bold text-white mr-3 border border-zinc-700 group-hover:border-cred-accent/50 transition-colors rounded-full shrink-0">
                  {member.name.substring(0,2).toUpperCase()}
                </div>
                <div className="flex flex-col overflow-hidden">
                  <span className="text-sm font-medium truncate text-zinc-300 group-hover:text-white mb-0.5">{member.name}</span>
                  <div className="flex flex-col">
                    <span className="text-sm font-mono font-bold leading-none text-zinc-400">
                      {timeStr}
                    </span>
        ...
        <div className="p-4 border-t border-cred-border bg-black/50 shrink-0">
          <button 
            onClick={onAddMember}
            className="w-full h-10 flex items-center justify-center bg-zinc-900 border border-zinc-700 hover:border-cred-accent hover:bg-zinc-800 text-white transition-all group rounded-sm"
          >
            <Plus size={20} className="text-zinc-400 group-hover:text-cred-accent transition-colors" />
        ...
        <div style={{ width: totalWidth, height: '100%' }} className="relative bg-[linear-gradient(to_right,#18181b_1px,transparent_1px)] bg-[size:120px_100%]">
                 ...
                 <div key={i} className="absolute h-full border-l border-zinc-800" style={{ left: dayPx, width: 24 * 60 * PIXELS_PER_MINUTE }}>
                    ...
                    {Array.from({ length: 24 }).map((_, h) => (
                      <div key={h} className="absolute bottom-0 h-3 border-l border-zinc-800" style={{ left: h * 60 * PIXELS_PER_MINUTE }}>
                         <span className="absolute bottom-4 -left-2 text-[10px] font-mono text-zinc-600">
                           {h === 0 ? '' : h}
                         </span>
                      </div>
                    ))}
          ...
          <div className="relative">
            ...
            {members.map((member, idx) => (
              <div key={member.id} className="h-20 border-b border-cred-border/30 relative hover:bg-white/5 transition-colors group">
          ...
          <div 
            ...
            style={{ left: nowPx }}
          >
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-cred-accent text-black text-[9px] font-bold px-1.5 py-0.5 rounded-sm font-mono whitespace-nowrap">
          ...
          {selectionStartX !== null && currentMouseX !== null && overlayWidth > 0 && (
            <div 
              className="absolute top-[60px] bottom-0 bg-cred-accent/10 border-x border-cred-accent/30 z-20 pointer-events-none backdrop-blur-[1px]"
              style={{
                left: overlayLeft,
                width: overlayWidth
              }}
          ...
          {selectionStartX !== null && currentMouseX !== null && overlayWidth > 0 && (
             <div 
                className="absolute top-[80px] z-50 bg-black/90 text-cred-accent text-xs px-3 py-1.5 border border-cred-border font-mono whitespace-nowrap rounded-sm shadow-[0_4px_20px_rgba(0,0,0,0.5)] pointer-events-none flex items-center justify-center min-w-[60px]"
                style={{
                  left: currentMouseX,
      ...
      {hoveredDate && mousePos && (
        <div 
          className="fixed z-50 pointer-events-none bg-black/80 backdrop-blur-xl border border-zinc-800 rounded-lg p-3 shadow-2xl flex flex-col gap-2 min-w-[200px]"
          style={{ 
            top: mousePos.y + 20, 
